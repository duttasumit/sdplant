$(document).ready(function(){
    ...
    $('body').on('click','#go',function(){
        ...
        if((num_of_activities>=0) && (num_of_resources>=0) && (num_of_discussion>=0)){ 
			...
			for(cont in students){
				tree(students[cont],cont);				
				baskets[cont]=basket;
				for(cont in baskets){
				Basket(baskets[cont],cont);
				cont++;}}
	});
...
// login type and particular type
    $('body').on("change",".loginday, .submission,.viewed",function(){
		var num;
		($(this).parent().attr("class")=='right1')?num=1:num=2;
		...
		st_Score(students[num],value,num);
